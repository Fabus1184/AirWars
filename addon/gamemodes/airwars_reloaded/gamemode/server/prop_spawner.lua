...
function AirWars:SpawnProp(position, angle, category, index, player, stacker)
	...
	if prop["weapon"] then
		...
		if !player:HasWeapon(prop["weapon"]) then
			player:Give(prop["weapon"], true)
		end
		
		player:GiveAmmo(prop["amount"], prop["ammo"])
	else
		-- spawn aw_building_prop
		ent = ents.Create("aw_building_prop")
