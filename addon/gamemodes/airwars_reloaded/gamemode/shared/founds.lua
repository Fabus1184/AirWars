function build_calculate_ship_cost(team_id)
	...
	for _, player in pairs(player.GetAll()) do
		if player:GetAWTeam() != team_id then continue end
		
		for k, v in pairs(player:GetAmmo()) do
			for _, prop in pairs(global_config.categories[CATEGORY_DICKE_ZUGRIFF_WAFFEN].props) do
				if prop["ammo"] == k then
					cost = cost + (v * prop["info"].cost / prop["amount"])
					break
				end
			end
		end
